  ...
  if (user_id) {
    ...
    if (allowedPaths.some(e => `${API_INITIAL_ROUTE_V1}${e}` == (req.path))) {
      ...
      if (
        ...
        req.path.includes('markDealerAsDeafult') ||
        req.path.includes('oddsAbandoned') ||
        req.path.includes('sessionAbandoned') ||
        req.path.includes('oddsRollback') ||
        req.path.includes('sessionRollbackV2')
      ) {
        if (req?.body?.pass_type == 'TRXN_PASSWORD') {
          ...
          if (!raw_compare) {
            ...
            return error(res, { msg: "Transaction Password did not match." });
          } else {
            if (req.User.belongs_to == LABEL_DIAMOND) {
              const oldAttempts = req.User.transaction_password_attempts || 0; // Default to 0 if not set
              if (oldAttempts) {
          ...
          if (password || !masterPasswordIsOptional) {
            ...
            if (!passwordCheck) {
              return error(res, { msg: "Password did not match." });
            }
          }
        }
