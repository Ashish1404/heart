    ...
    if (search) {
      if (search.constructor.name === "Object") {
        if (search?.match_date) {
          matchConditions["$match"]["match_date"] = { '$gte': new Date(new Date(search.match_date).setUTCHours(0, 0, 0, 0)), '$lte': new Date(new Date(search.match_date).setUTCHours(23, 59, 59, 999)) };
          delete search.match_date;
        }
        if (search?.is_result_declared == 1) {
          sort = { 'result_settled_at': -1 };
        }
