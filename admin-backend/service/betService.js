          ...
          const updateMarket = await Market.updateOne(
            { market_id },
            {
              $set: {
                ...
                result_value_selections: selection_name,
    ...
    await session.withTransaction(async (session) => {
      ...
      if (status.statusCode == SUCCESS) {
        ...
        if (rollback) {
          ...
          // Update Bet Count Expire At
          updateBetCountExpire({ match_id: market.match_id, event_id: market_id });
        }
    ...
    await session.withTransaction(async (session) => {
      ...
      if (rollback) {
        ...
        try {
          ...
          // Update Bet Count Expire At
          updateBetCountExpire({ match_id: fancy.match_id, event_id: fancy_id });
...
async function updateBetCountExpire(filter) {
  ...
  BetCount
    .updateMany(filter, { $set: { expire_at, is_active: false, } })
