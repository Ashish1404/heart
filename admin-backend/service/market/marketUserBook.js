...
function buildHierarchy(inputArray) {
  ...
  inputArray = inputArray.sort((a, b) => a.sort_priority - b.sort_priority);
...
module.exports = {
  diamondUserBook: async function (req) {
    try {
      ...
      const Query = userBookQuery({
        market_id,
        user_id,
        agent_ids_to_exclued: Self.parent_level_ids.map(i => ObjectId(i.user_id)),
      });

      const oddsProfitLossData = await OddsProfitLoss.aggregate(Query).allowDiskUse(true);
