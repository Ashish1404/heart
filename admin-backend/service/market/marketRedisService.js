...
const { setBBBRates } = require("./setBBBRates");
const { setTWTTRates } = require("./setTWTTRates");
...
module.exports = {
  ...
  createManualRates: async (req) => {
    try {
      const { market_id } = req.joiData;

      const market = await Market.findOne({
        market_id, is_active: 1, is_visible: true, is_result_declared: 0, is_abandoned: 0
      }, {
        marketId: "$market_id", marketName: "$market_name"
      }).lean();

      if (!market) {
        return resultResponse(SERVER_ERROR, { msg: 'No Market found with this Market Id.' });
      }

      if (market.marketName === TO_WIN_THE_TOSS) {
        await setTWTTRates(market);
      }

      if (market_id == BALL_BY_BALL_EVENT_DATA.market_id) {
        await setBBBRates(market);
      }

      return resultResponse(SUCCESS, { msg: 'Rate Created Successfully! ' });
    } catch (error) {
