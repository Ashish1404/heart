...
async function updateForChangePasswordAfterLogin(req) {
  try {
    ...
    const user = await User.findOne({ _id: req.params.id }, { password: 1, is_change_password: 1 }).lean();
    ...
    const isFirstLogin = user.is_change_password == 0 ? true : false;
    ...
    if (isSelf && !isFirstLogin) {
      await OAuthToken.deleteMany({ 'user.user_id': req.params.id.toString() });
    } else if (!isSelf) {
      await OAuthToken.deleteMany({ 'user.user_id': req.params.id.toString() });
    }
...
async function selfChangePassword(data) {
  try {
    ...
    if (is_self) {
      const userData = await User.findOne({ _id: user._id }, { is_change_password: 1 }).lean();
      if (userData.is_change_password == 0) {
        isFirstLogin = true;
      }
    }
    ...
    if (path == "/changeChildPassword") {
      updateObj.$set.is_change_password = 0;
    }
