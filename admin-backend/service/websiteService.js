...
async function updateDefaultPopUpSetting(req) {
  const { WebsiteSetting } = req;

  WebsiteSetting.is_default_popup =
    !WebsiteSetting.is_default_popup;
  ...
  return resultResponse(SUCCESS, {
    msg: `Default popup image on the domain ${
      WebsiteSetting.domain_name
    } have been successfully ${
      WebsiteSetting.is_default_popup ? "enabled" : "disabled"
    }.`,
    is_default_popup: WebsiteSetting.is_default_popup,
  });
}
