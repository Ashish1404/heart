const { FANCY_DUMP_CRON_STATUS,
  ...
  ENABLE_AUTO_MARKET_RATES_SUSPEND,
  ODDS_API_PROVIDER,
...
async function restAPIconnect(io) {
  ...
  async function init_inplay() {
    ...
    if (ODDS_API_PROVIDER == 'DM_PROVIDER') {
      const sportIds = ["4"];
      const sportIdsOthers = ["2", "1",];
      let orArray = [];
      const racingOrFilter = [
        { sport_id: { $nin: ["7", "4339"] } },
        { market_id: { $regex: ".+(?<!_m)$" } }
      ];

      if (RACING_ODDS_API_PROVIDER == 'BETFAIR') {
        const filter = {
          cron_inplay: true,
          sport_id: { "$in": ["7", "4339"] },
        }
        tasks.push(getMarkets(filter, INPLAY, 3, CricketCode, { providerType: 'BETFAIR' }));
      } else {
        sportIdsOthers.push(...["7", "4339"]);
        orArray = racingOrFilter;
      }

      const filter = {
        cron_inplay: true,
        sport_id: { "$in": sportIds },
        market_type: { '$ne': BOOKMAKER_TYPE },
      }

      const filterOther = {
        cron_inplay: true,
        sport_id: { "$in": sportIdsOthers },
        ...(orArray?.length ? { $or: orArray } : {}),
      }
      tasks.push(
        getMarkets(filter, INPLAY, 3, CricketCode, { providerType: 'DM_PROVIDER' }),
        getMarkets(filterOther, INPLAY, 3, CricketCode, { providerType: 'DM_PROVIDER_OTHER' })
      );
    } else {
      ...
      await getMarkets({ cron_inplay: true, sport_id: "4", market_type: { '$ne': BOOKMAKER_TYPE } }, INPLAY, 3, CricketCode, { providerType: 'BETFAIR' });
      if (isShowLogs) {
      ...
      await getMarkets({ cron_inplay: true, sport_id: { "$in": ["2", "1"] } }, INPLAY, 3, SocTenCode, { providerType: 'BETFAIR' });
      if (isShowLogs) {
      ...
      await getMarkets({ cron_inplay: true, sport_id: { "$in": ["7", "4339"] } }, INPLAY, 3, HrGHrCode, { providerType: 'BETFAIR' });
      if (isShowLogs) {
    ...
    tasks.push(
      getMarkets({ cron_inplay: true, sport_id: "4", market_type: BOOKMAKER_TYPE, match_date: { "$gte": today }, market_id: { '$regex': /\d*\.\d*/ } },
        BOOKMAKER_TYPE, 3, { providerType: 'BOOKMAKER' }),
    );
  ...
  async function init_delay() {
    ...
    if (ODDS_API_PROVIDER == 'DM_PROVIDER') {
      ...
      if (RACING_ODDS_API_PROVIDER == 'BETFAIR') {
        ...
        tasks.push(getMarkets(filter, DELAY, 15, '', { providerType: 'BETFAIR' }));
      } else {
        sportIdsOthers.push(...["7", "4339"]);
        orArray = racingOrFilter;
      }
      ...
      tasks.push(
        getMarkets(filter, DELAY, 15, '', { providerType: 'DM_PROVIDER' }),
        getMarkets(filterOther, DELAY, 15, '', { providerType: 'DM_PROVIDER_OTHER' }),
      );

    } else {
      await getMarkets({ cron_inplay: false, sport_id: "4", market_type: { '$ne': BOOKMAKER_TYPE } }, DELAY, 15, '', { providerType: 'BETFAIR' });

      await getMarkets({ cron_inplay: false, sport_id: "2" }, DELAY, 15, '', { providerType: 'BETFAIR' });

      await getMarkets({ cron_inplay: false, sport_id: "1" }, DELAY, 15, '', { providerType: 'BETFAIR' });

      await getMarkets({ cron_inplay: false, sport_id: "7", market_id: { $regex: ".+(?<!_m)$" } }, DELAY, 15, '', { providerType: 'BETFAIR' });

      await getMarkets({ cron_inplay: false, sport_id: "4339", market_id: { $regex: ".+(?<!_m)$" } }, DELAY, 15, '', { providerType: 'BETFAIR' });
    }

    var today = new Date();
    today.setDate(today.getDate() - 5);
    tasks.push(
      getMarkets({ cron_inplay: false, sport_id: "4", market_type: BOOKMAKER_TYPE, match_date: { "$gte": today }, market_id: { '$regex': /\d*\.\d*/ } },
        BOOKMAKER_TYPE, 15, '', { providerType: 'BOOKMAKER' })
    );
  ...
  async function getMarkets(data, API_TYPE, EXPIRE, UUID = undefined, extras = {}) {

    let providerType = extras.providerType || "";
    ...
    if (markets.statusCode == SUCCESS) {
      ...
      let markets_data = await marketService.getOddsRates({ markets_ids: MarketsIds, sport_id: data.sport_id, API_TYPE, chunkSize, providerType });
      if (API_TYPE == INPLAY && isShowLogs) {
