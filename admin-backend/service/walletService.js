...
const mongoose = require('mongoose')
  ...
  , {
    ...
    ACCOUNT_STATEMENT_TYPE_BONUS, USER_TYPE_WHITE_LABLE
  } = require('../../utils/constants')
  , { removeStaticContent, generateReferCode, exponentialToFixed, delay } = require('../../utils')
...
let walletagentsAndUsersCr = async (data, userDetail) => {
  ...
  try {
    ...
    if (dealerData) {
      // Initialize walletagents with dealerData
      walletagents = [dealerData];
      // Find all users with user_type_id: 8
      let agents = userDetail.parent_level_ids.filter(user => user.user_type_id == USER_TYPE_WHITE_LABLE).map(user => user.user_id);
      // Push these agent IDs into walletagents
      walletagents.push(...agents);
  ...
  try {
    ...
    await session.withTransaction(async session => {
      try {
        ...
        const res1 = await chipwalletInOut(data.req.body, userData, data.res);

        if (res1 != 1) {
          throw new Error("Some Error in User Deposit/WithDraw");
        }
        // Bonus
        if (bonus_amount) {
          data.req.body.user_id = statement_preview.user_id;
          ...
          const userData = await User.findOne({ '_id': ObjectId(statement_preview.user_id) }).session(session);
          const res2 = await chipwalletInOut(data.req.body, userData, data.res);

          if (res2 != 1) {
            throw new Error("Some Error in User Bonus Deposit/WithDraw");
          }
        }
