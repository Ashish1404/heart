...
let agentsAndUsersCrDr = async (data, userDetail) => {
  const isOutterSession = Boolean(data.session);
  const session = isOutterSession ? data.session : await mongoose.startSession();
  const transactionOptions = {
    readPreference: 'primary',
    readConcern: { level: 'majority' },
    writeConcern: { w: 'majority' }
  ...
  try {
    ...
    if (data.crdr === CREDIT_ONE) {
      ...
      if (isOutterSession) {
        ...
        await session.withTransaction(task, transactionOptions);
      }
