...
async function resetPassword(data) {
    try {
        ...
        if (data.joiData.country_code)
            delete data.joiData.country_code
        const existingMobile = await User.findOne(data.joiData).select(`mobile country_code is_verified -_id`).lean();
        if (existingMobile && Object.keys(existingMobile).length > 0) {
            const response = await sendCode(existingMobile);
            if (response.status == 200) {
                await User.updateOne(data.joiData, { orderId: response.data.orderId, is_verified: 0 });
                return resultResponse(CONSTANTS.SUCCESS, {
                    data: response.data, mobile: existingMobile.mobile,
                    msg: "Successfully send OTP on your Whatsapp number.",
            ...
            return resultResponse(CONSTANTS.NOT_FOUND, { msg: "Mobile number not found, Please contact upper level." });
        }
    } catch (error) {
        return resultResponse(CONSTANTS.SERVER_ERROR, error);
    }
}
    ...
    try {
        ...
        if (response) {
            if (response.data !== undefined && response.data.isOTPVerified) {
                ...
                return resultResponse(CONSTANTS.VALIDATION_ERROR, { msg: "Incorrect OTP!" });
            }
        }
    } catch (error) {
        return resultResponse(CONSTANTS.SERVER_ERROR, error);
...
async function setPassword(req) {
    try {
        ...
        if (!user)
            return resultResponse(CONSTANTS.VALIDATION_ERROR, { msg: "User not found or otp not verified." });
