const { ObjectId } = require("bson")
  ...
  , {
    ...
    GET_MANUAL_ODDS_API_INPLAY, REMOVED, WINNER, LOSER, INPLAY, DELAY, HR, GHR,
...
const {
  ...
  GET_ODDS_API_INPLAY,
  GET_ODDS_API_DELAY,
  GET_ODDS_API_DM_PROVIDER_OTHER,
  DM_PROVIDER_CRICKET_EVENT,
  DM_PROVIDER_OTHER_EVENT,
  GET_ODDS_API_INPLAY_BETFAIR,
  GET_ODDS_API_DELAY_BETFAIR,
  GET_ODDS_API_DM_PROVIDER,
  RACING_ODDS_API_PROVIDER, } = require('../../config/constant/rateConfig');
...
const { setBBBRates } = require('./market/setBBBRates');
...
function getMarketAgentUserPositions(params, parents) {
  ...
  let FilterQuery = { parent_id: user_id, is_demo: { $ne: true } };
  if (master_book)
    ...
    if (getActiveMarketIds.length) {
      ...
      odds.map(async (data, index) => {
        if (data != null) {
          ...
          if (ODDS_API_PROVIDER == 'DM_PROVIDER') {
            const marketFromDB = markets.find(market => market.market_id == marketId);
            if (marketFromDB) {
              if (marketFromDB.market_type == BOOKMAKER_TYPE) {
                const matchTime = moment(marketFromDB.match_date);
                const now = moment();
...
async function marketOddsServiceForCoreSports() {
  ...
  const getActiveMarkets = await Market.find(Filter, { _id: 0, match_id: 1, market_id: 1, runners: 1, sport_id: 1, market_type: 1, match_date: 1 }).lean();
...
async function autoMinMaxLimitUpdateDmProvider() {
  let Filter = {
    ...
    $or: [
      { market_live_odds_validation: true },
      { session_live_odds_validation: true },
    ]
  };
  const getActiveMatches = await Match.find(Filter, {
    _id: 0, match_id: 1, market_id: 1, sport_id: 1,
    market_live_odds_validation: 1,
    session_live_odds_validation: 1,
  }).lean();
  processMatchesForAutoUpdateLimit(getActiveMatches);
}

async function processMatchesForAutoUpdateLimit(getActiveMatches) {
  try {
    const cricketMatches = [];
    const otherSportMatches = [];
    getActiveMatches.map(i => {
      if (i.sport_id == CONSTANTS.CRICKET) {
        cricketMatches.push(i);
      } else {
        otherSportMatches.push(i);
      }
    });

    const cricketMarketsObj = await getLiveLimitFromDmAPI(cricketMatches, true);
    const otherMarketsObj = await getLiveLimitFromDmAPI(otherSportMatches, false);

    let marketsMerged = {};
    let fanciesMerged = {};
    if (cricketMarketsObj.statusCode == SUCCESS) {
      marketsMerged = { ...marketsMerged, ...(cricketMarketsObj?.data?.markets || {}) };
      fanciesMerged = { ...fanciesMerged, ...(cricketMarketsObj?.data?.fancies || {}) };
    }

    if (otherMarketsObj.statusCode == SUCCESS) {
      marketsMerged = { ...marketsMerged, ...(otherMarketsObj?.data?.markets || {}) };
    }

    const bulkWriteMarket = Object.values(marketsMerged).map((data) => {
      return {
        updateOne: {
          filter: { market_id: data.marketId },
          update: {
            $set: {
              live_market_min_stack: data.min,
              live_market_max_stack: data.max,
              news: data.news,
            }
          }
        }
    ...
    const bulkWriteFancies = Object.values(fanciesMerged).map((data) => {
      return {
        updateOne: {
          filter: { fancy_id: data.marketId },
          update: {
            $set: {
              session_live_min_stack: data.min,
              session_live_max_stack: data.max,
            }
          }
        }
      }
    ...
    if (bulkWriteMarket.length) {
      await Market.bulkWrite(bulkWriteMarket, { ordered: false });
    }
    if (bulkWriteFancies.length) {
      await Fancy.bulkWrite(bulkWriteFancies, { ordered: false });
    }

  } catch (error) {
    console.error("Error in processMatchesForAutoUpdateLimit: ", error.message);
  }
}

async function getLiveLimitFromDmAPI(activeMatches, isCricket = true) {
  try {
    const markets = {}, fancies = {};
    const marketKeys = {
      [CONSTANTS.CRICKET]: ['match_odd', 'completed_match', 'tied_match', 'bookmaker'],
      [CONSTANTS.SOCCER]: ['match_odd', 'goals'],
      [CONSTANTS.TENNIS]: ['match_odd'],
      ['77']: ['horse_racing'],
      [CONSTANTS.GHR]: ['grey_hound_racing']
    }
    const fancyKeys = {
      [CONSTANTS.CRICKET]: ['session'],
    }

    const url = isCricket ? DM_PROVIDER_CRICKET_EVENT : DM_PROVIDER_OTHER_EVENT;

    if (activeMatches.length == 0) {
      return resultResponse(SERVER_ERROR, { markets, fancies });
    }
    const match_ids = activeMatches.map(i => i.match_id).join(",");
    let data = (await axios.get(url + match_ids, { timeout: 10000 }))?.data;

    if (!data?.gameList?.length) {
      return resultResponse(SERVER_ERROR, { markets, fancies });
    }

    for (const gameListItem of data.gameList) {
      const { sportId, eventList } = gameListItem;

      if (!eventList?.length) {
        continue;
      }

      for (const eventListItem of eventList) {
        const { marketList } = eventListItem;
        const keys = marketKeys[sportId] || [];
        ...
        for (const key of keys) {
          ...
          for (const market of data) {
            markets[market.marketId] = {
              ...
              min: market.min_stack,
              max: market.max_stack,
              max_profit_limit: market.max_profit_limit,
              news: market?.info || undefined,
            }
          }
        }

        // Fancies
        const keys2 = fancyKeys[sportId] || [];
        for (const key of keys2) {
          ...
          for (const market of data) {
            ...
            const SelectionId = marketId.split('-')[0] ? marketId.split('-')[0].split('.')[1] : 'undefined';
            fancies[market.marketId] = {
              marketId: `${market.eid}_${SelectionId}`,
              min: market.min_stack,
              max: market.max_stack,
              max_profit_limit: market.max_profit_limit,
            }
    ...
    return resultResponse(SUCCESS, { markets, fancies });

  } catch (error) {
    console.error("Error in getLiveLimitFromDmAPI: ", error.message);
...
async function inactiveAutoMarkets(provider) {
  try {
    if (provider == "frnk") {
      ...
      let MarketsIds = [], racingMarketIds = [];

      markets.map(data => {
        if (data.sport_id != "4" || data.market_type != WINNER) {
          if (data.sport_id == HR || data.sport_id == GHR) {
            racingMarketIds.push(data.market_id);
          } else {
            MarketsIds.push(data.market_id);
          }
        }
      });

      let markets_data = [], chunkSize = getChunkSize(INPLAY), chunkSizeDelay = getChunkSize(DELAY);
      const resultMatchIds = [];

      if (ODDS_API_PROVIDER == 'DM_PROVIDER' && RACING_ODDS_API_PROVIDER != 'DM_PROVIDER') {
        let markets_live = await getOddsRates({ markets_ids: racingMarketIds, API_TYPE: INPLAY, chunkSize, providerType: 'BETFAIR' });
        if (markets_live.statusCode == SUCCESS)
          markets_data.push(...markets_live.data);

        let markets_delay = await getOddsRates({ markets_ids: racingMarketIds, API_TYPE: DELAY, chunkSize: chunkSizeDelay, providerType: 'BETFAIR' });
        if (markets_delay.statusCode == SUCCESS)
          markets_data.push(...markets_delay.data);
      } else {
        MarketsIds.push(...racingMarketIds);
      }
      ...
      let markets_delay = await getOddsRates({ markets_ids: MarketsIds, API_TYPE: DELAY, chunkSize: chunkSizeDelay });
      if (markets_delay.statusCode == SUCCESS)
        markets_data.push(...markets_delay.data);
      ...
      for (const data of markets_data) {
        ...
        if (!data?.runners?.length) continue;
        ...
        if (!resultStatus?.length) continue;
        ...
        const matchId = markets.find(market => market.market_id == data.marketId)?.match_id;
        if (matchId)
...
async function getResult(data) {
  ...
  const url = (await apiUrlSettingsService.getMarketResultUrl()) + market_id + "&central_id=" + market.centralId;
...
let getOddsAPI = async (data) => {
  try {
    ...
    let markets_ids = data.markets_ids,
      { API_TYPE, providerType } = data;

    let GET_ODDS_API_BOOKMAKER = API_TYPE == BOOKMAKER_TYPE
      ? await apiUrlSettingsService.getBaseUrl() + "/getOdds?function_name=" + methodName + "&ids="
      : "";
    ...
    if (providerType) {
      ...
      switch (providerType) {
        case "BETFAIR": url = API_TYPE == INPLAY
          ? GET_ODDS_API_INPLAY_BETFAIR
          : GET_ODDS_API_DELAY_BETFAIR;
          break;
        case "DM_PROVIDER": url = GET_ODDS_API_DM_PROVIDER;
          break;
        case "DM_PROVIDER_OTHER": url = GET_ODDS_API_DM_PROVIDER_OTHER;
          break;
        case "BOOKMAKER": url = GET_ODDS_API_BOOKMAKER;
          break;
        default: url = "";
      }
      ...
      if (API_TYPE == INPLAY) {
        url = GET_ODDS_API_INPLAY;
      } else if (API_TYPE == DELAY) {
        url = GET_ODDS_API_DELAY;
      } else if (API_TYPE == BOOKMAKER_TYPE) {
        url = GET_ODDS_API_BOOKMAKER;
      }
    }
    ...
    // console.error("Error in getOddsAPI: ", error);
    return resultResponse(SERVER_ERROR, error.message);
  }
}
...
async function setOddsAPIResponse(params) {
  try {
    ...
    switch (params.API_TYPE) {
      ...
      default: {
        if (params?.providerType == 'BETFAIR') {
          if (response.length) {
            return resultResponse(SUCCESS, postProcessMarketAPIData(response));
          }
        }
        if (ODDS_API_PROVIDER == 'DM_PROVIDER') {
          const isOther = params?.providerType == 'DM_PROVIDER_OTHER';
          if (response?.data?.items?.length || isOther && response?.data?.items) {
            return resultResponse(SUCCESS, dmProviderFormat(response, isOther));
          }
        } else {
          if (response.length) {
            return resultResponse(SUCCESS, postProcessMarketAPIData(response));
...
function dmProviderFormat(response, isOtherSports = false) {
  try {
    let items = response?.data?.items;

    if (isOtherSports) {
      items = Object.keys(items).flatMap(i => items[i]);
    }
    if (items.length) {
      return items.filter(i => i).map(item => {
        const { market_id, inplay, odds } = item;
        let status = item.status;
        let runnersOutter = [];

        if (item.odds) {
          runnersOutter = odds.map(odd => {
            const { selectionId, status } = odd;
            function temp(value) {
            ...
            for (let i = 1; i <= 3; i++) {
              availableToBack.push({
                "price": temp(odd[`backPrice${i}`]),
                "size": temp(odd[`backSize${i}`]),
              })

              availableToLay.push({
                "price": temp(odd[`layPrice${i}`]),
                "size": temp(odd[`laySize${i}`]),
              })
            }
            return {
              selectionId,
        ...
        } else if (item.runners) {
          const runners = item.runners;
          const { suspended, ballRunning } = item;
          status = suspended ? 'SUSPENDED' : ballRunning ? "BALL_RUNNING" : 'OPEN';
          runnersOutter = runners.map(odd => {
            const { secId, suspended, ballRunning } = odd;
            let status = suspended ? 'SUSPENDED' : ballRunning ? "BALL_RUNNING" : 'ACTIVE';
            ...
            for (let i = 1; i <= 3; i++) {
              availableToBack.push({
                "price": i == 1 ? temp(odd[`backPrice`] || odd[`back`]) : 0,
                "size": i == 1 ? temp(odd[`backSize`]) : 0,
              })

              availableToLay.push({
                "price": i == 1 ? temp(odd[`layPrice`] || odd[`lay`]) : 0,
                "size": i == 1 ? temp(odd[`laySize`]) : 0,
              })
            }
            return {
              selectionId: secId,
        ...
        return {
          "marketId": market_id,
          status: (status == 'OPEN' || status == 'ACTIVE')
            ? runnersOutter.every(runner => runner.status == 'SUSPENDED')
              ? 'SUSPENDED'
              : 'OPEN'
            : status,
          inplay: inplay == true || inplay == 'IN_PLAY' ? true : false,
          // "max": 500000,
          // "min": 100,
          "news": "",
          runners: runnersOutter,
        }
      })
    } else {
      return [];
...
module.exports = {
  ...
  setTWTTRates,
  autoMinMaxLimitUpdateDmProvider,
  setBBBRates,
  createManualRates,
};