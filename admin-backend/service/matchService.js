...
const {
  ENABLE_SCORE_IN_MATCH_DETAILS,
  ENABLE_SCORE_IN_MATCH_DETAILS_V2,
  ODDS_API_PROVIDER, } = require('../../config/constant/rateConfig');
...
const { fetchAndSaveScoreData, getDMScoreData } = require("./match/score-service");
const { userCount, totaluserCount } = require("./match/user-count-service");
...
async function homeMatchesV2(data) {
  ...
  try {
    ...
    if (matchData.length) {
      ...
      if (combine) {
        await combineMarketsV2(matchData.filter(i => i.market_id != BALL_BY_BALL_EVENT_DATA?.market_id));
      }
...
async function homeMatches(data) {
  try {
    ...
    let Fields = [
      "-_id", "sport_name", "series_name", "market_id", "marketId", "status", "inplay",
    ...
    if (combine) {
      Fields.push(...["runners.selectionId", "runners.status", "runners.ex"]);
    }
...
async function matchDetails(data) {
  try {
    ...
    let Fields = {
      _id: 0,
      market_id: 1, name: 1, market_name: 1, status: 1, inplay: 1, match_date: 1, enable_fancy: 1,
    ...
    if (result.length) {
      ...
      if (ENABLE_SCORE_IN_MATCH_DETAILS == 'true' && ODDS_API_PROVIDER == 'DM_PROVIDER') {
...
async function matchDetailsWrapperV2(req) {
  try {
    const { combine_fancy, category_wise_fancy, match_id } = req.body;
    ...
    if (combine_fancy) {
      // Get Fancy Data if combine fancy is true
      tasks.push(getFanciesV2(req));
    }

    if (ENABLE_SCORE_IN_MATCH_DETAILS_V2 == 'true' && ODDS_API_PROVIDER == 'DM_PROVIDER') {
      // Get DM Score Data if enabled
      const scoreDataResponse = await getDMScoreData(match_id);
      tasks.push(scoreDataResponse);
    }

    const response = await Promise.all(tasks);

    if (!response.length) {
      return resultResponse(SERVER_ERROR, "Something Went Wrong !!");
    }

    const request = req;
    return buildMatchResponse(request, response);
...
const buildMatchResponse = (request, response) => {
  const [matchDetailsRes, fancyRes, scoreDataRes] = normalizeResponseArray(response);

  const isMatchSuccess = matchDetailsRes?.statusCode === SUCCESS;
  const isFancySuccess = fancyRes?.statusCode === SUCCESS;
  const isScoreSuccess = scoreDataRes?.statusCode === SUCCESS;

  const matchData = matchDetailsRes?.data || [];
  const fancyData = fancyRes?.data || [];
  const scoreData = isScoreSuccess ? scoreDataRes?.data || {} : null;

  const check_event_limit = request.User.check_event_limit;

  if (!isMatchSuccess && !isFancySuccess) {
    return resultResponse(NOT_FOUND, "No data found");
  }
  ...
  if (isMatchSuccess) {
    result.markets = matchData;
  }

  if (isFancySuccess) {
    result.fancies = fancyData;
  }
  ...
  result.check_event_limit = check_event_limit;
  return resultResponse(SUCCESS, result);
};

const normalizeResponseArray = (resArr) => {
  if (resArr.length === 3) return resArr;
  if (resArr.length === 2) {
    // market,fancy | market,score
    const [first, second] = resArr;
    const isScore = second?.data?.isScore;
    return isScore ? [first, null, second] : [first, second, null];
  }
  if (resArr.length === 1) return [resArr[0], null, null];
...
async function matchDetailsOpen(data) {
  const FilterQuery = {
    ...
    _id: 0, market_id: 1, market_name: 1, status: 1, match_date: 1, market_min_stack: 1, market_max_stack: 1, matched: 1,
