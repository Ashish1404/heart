...
async function userEventProfitLoss({ user_id, search, from_date, to_date, page = 1, limit = 50 }) {
  ...
  const filter = { user_id, is_demo: false };
  if (from_date && to_date) {
...
function sportsWiseUsersPL(params) {
  const { user_id, user_type_id, user_name, search, international_casinos, loggedin_user_type_id } = params;
  let query = {};
  if (user_id)
    ...
    if (usersByParentId.statusCode == SUCCESS) {
      ...
      if ((user_type_id - 1) == USER_TYPE_USER) {
        return Promise.all([
          ...
          data = groupUsersData(data, loggedin_user_type_id);
...
function groupUsersData(data, loggedin_user_type_id = null) {
  let sports = {};
  let users = data.reduce((prev, current) => {
    ...
    if (!found) {
      ...
      if (loggedin_user_type_id == USER_TYPE_SUPER_ADMIN) {
        tempObject.domain_name = current?.domain_name;
      }
...
async function sportsWiseUsersPLDiamond(params) {
  try {
    // Fetching the data from the report service (assuming it's a method you already have)
    const result = await sportsWiseUsersPL(params);
    ...
    const userNames = userList.map(user => user.user_name);

    // Fetch users from the database based on the user_names
    const dbUsers = await User.find({ user_name: { $in: userNames } }, { user_name: 1, share: 1 }).lean();

    // Map user_name => diamond_share
    const diamondMap = {};
    dbUsers.forEach(user => {
      diamondMap[user.user_name] = user.share || 0;
    });

    // Add diamond_share to each user in the response
    result.data.users = userList.map(user => ({
      ...user,
      diamond_share: diamondMap[user.user_name] || 0
    }));
...
async function extractUserParams(req) {
  try {
    const params = req.joiData;
    let { user_id, user_name } = params;
    let parent_id, user_type_id;
    ...
    if (!user_id) {
      ...
      user_id = ObjectId(user_id);
      parent_id = req.user?.parent_id || req.User?.parent_id;
      user_name = req.user?.user_name || req.User?.user_name;
      user_type_id = req.user?.user_type_id || req.User?.user_type_id;
    }

    params.user_id = user_id;
    params.user_type_id = user_type_id;
    params.user_name = user_name;
    params.loggedin_user_type_id = req.User.user_type_id;
    return { params, parent_id, user_name };
  } catch (error) {
    throw error
  }
};
