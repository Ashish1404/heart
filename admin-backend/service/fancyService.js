const { ObjectId } = require("bson")
  ...
  , EventsWiseLimites = require("../../models/eventWiseLimit")
...
const GET_FANCY_ODDS_API_INPLAY_DM_PROVIDER = process.env.GET_FANCY_ODDS_API_INPLAY_DM_PROVIDER;
  ...
  return getFancyBetForUserPosition(user_id, fancy_id, user_type_id, bet_id).then(fancyList => {
    ...
    if (fancyList.statusCode === SUCCESS) {
      ...
      if (fancyListData.length > 0) {
        ...
        data = {
          "fancy_position": resultValues,
          "liability": Number(max_exposure.toFixed(2)),
          "profit": Number(max_profit.toFixed(2)),
          stack_sum,
          bets_fancies: fancyListData
        };
      } else
...
let createFancyLiability = (data, params) => {
  ...
  for (const fancy of fancies) {
    ...
    let run = [], resultValues = [], orgRun = [];
    let lastPosition = 0, max_exposure = 0, max_exposure_full = 0, max_full_exposure = 0, max_profit = 0, max_full_profit = 0, stack_sum = 0;
    for (let i in fancyListData) {
      let fancy = fancyListData[i];
      stack_sum += fancy.stack;
      run.push(fancy.run - 1);
    ...
    run.map(function (r, ind) {
      ...
      if (max_exposure > tempTotal)
        max_exposure = tempTotal;
      if (max_exposure_full > tempFullTotal)
        max_exposure_full = tempFullTotal;
    ...
    FancyLiability[`${fancy_id}_full`] = max_exposure_full;
  }
  if (exposure)
    return exposureData;
  else
...
async function autoImportFancy() {
  ...
  try {
    ...
    let matches = await Match.find({ is_active: 1, is_visible: true, is_result_declared: 0, sport_id: "4", enable_fancy: 1, is_abandoned: 0 })
      .select("_id match_id match_date session_min_stack session_max_stack session_max_profit is_series_limit_applicable series_id");
    if (matches.length) {
      ...
      for (const match of matches) {
        ...
        if (fanciesByMatchId.statusCode == SUCCESS) {
          ...
          fanciesByMatchId = fanciesByMatchId.map(data => {
            const { sport_id, sport_name, series_id, series_name, match_id, match_name,
              selection_id, name, fancy_name, centralId, category, chronology, is_manual,
              Min, Max } = data;
            ...
            return {
              ...
              category_name,
              session_live_min_stack: Min,
              session_live_max_stack: Max,
            }
          });
          ...
          if (match.is_series_limit_applicable) {
            // Step 1: Extract unique categories from faciesNotInDB
            const uniqueCategories = [...new Set(faciesNotInDB.map(fancy => fancy.category_name))];

            // Step 2: Fetch limits for all unique categories at once
            const limitsData = await EventsWiseLimites
              .find({
              ...
              .select({
                ...
                category_name: 1 // Ensure `category_name` is included to match with `faciesNotInDB`
              })
              .lean()
              .exec();
            ...
            limitsData.forEach(limit => {
              limitsMap[limit.category_name] = {
                session_min_stack: limit.session_min_stack,
                session_max_stack: limit.session_max_stack,
                session_max_profit: limit.session_max_profit
              };
            });

            // Step 4: Apply settings based on category lookup
            faciesNotInDB = faciesNotInDB.map(fancy => ({
              ...fancy,
              ...(limitsMap[fancy.category_name] || {}), // Apply the fetched limits if available
              is_series_limit_applicable: true // Ensure this is always true when match.is_series_limit_applicable is true
            }));
          }
    ...
    if (provider == API_PROVIDER) {
      ...
      if (matches.statusCode == SUCCESS) {
        ...
        for (const match of matches) {
          ...
          if (!Array.isArray(fancies)) {
            return;
          } else if (!fancies.length) {
            fancies = [];
          }
          ...
          const filterObj2 = {
            ...
            ...(fanciesIds.length ? { fancy_id: { $nin: fanciesIds } } : {}),
                ...
                Fancy.updateOne(
                  { fancy_id },
                  {
                    '$set': {
                      ...
                      ...(
                        PROVIDER_API_FANCY == "DM_PROVIDER"
                          ? {}
                          : {
...
async function getFancyD247API(data) {
  try {
    ...
    try {
      ...
      console.error("GET_FANCY_ODDS_API_INPLAY_d247 failed", error.message);
    }
  } catch (error) {
    return resultResponse(SERVER_ERROR, error.message);
  }
...
async function getFancyPARKER777API(data) {
  try {
    ...
    try {
      ...
      console.error("GET_FANCY_ODDS_API_INPLAY_PARKER777 failed", error.message);
    }
  } catch (error) {
    return resultResponse(SERVER_ERROR, error.message);
  }
...
async function getFancyDmProviderAPI(data) {
  try {
    let { id, timeout } = data;
    let response;
    // console.log("getFancyDmProviderAPI: ", id)
    const fancies = await Fancy.find({ match_id: id, is_active: 1 }, {
      fancy_id: 1,
      selection_id: 1,
      session_live_min_stack: 1,
      session_live_max_stack: 1
    }).lean();
    ...
    response = await axios.get(GET_FANCY_ODDS_API_INPLAY_DM_PROVIDER + id, { timeout });

    const items = response?.data?.data?.items[id];
    if (items) {
      ...
      keys.map(key => {
        if (key == 'fancy2') {
          ...
          const fancyIds = Object.keys(data);

          if (!fancyIds.length) return;

          fancyIds.map(fancyId => {
            const item = data[fancyId];
            const { event_id, market_id, title, suspended, ballrunning, ball_running, bet_allowed } = item;
            const { yes, no, yes_rate, no_rate } = item;

            const SelectionId = market_id.split('-')[0]
              ? market_id.split('-')[0].split('.')[1]
              : 'undefined';

            const fancyData = fancies.find(i => i.selection_id == SelectionId);
            if (!fancyData) return;
            ...
            finalData.push({
              ...
              "backLayLength": 1,
              "GameStatus": status,
              "MarkStatus": bet_allowed == undefined || bet_allowed == 1 ? "0" : "1",
              "Min": fancyData.session_live_min_stack,
              "Max": fancyData.session_live_max_stack,
              "fancy_id": `${event_id}_${SelectionId}`
            })
          });
        }
      });
    ...
    console.error("GET_FANCY_ODDS_API_INPLAY_DM_PROVIDER failed", error.message);
    return resultResponse(SERVER_ERROR, error.message);
  }
}
...
async function getMethodName(provider) {
  ...
  switch (provider) {
    ...
    case "PARKER777":
      ...
      break;
    case "DM_PROVIDER":
      methodName = getFancyDmProviderAPI;
      break;
    default:
