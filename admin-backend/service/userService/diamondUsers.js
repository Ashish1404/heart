...
module.exports.getClientPlSum = async function getClientPlSum(request) {
  try {
    // Destructure request for commonly used properties
    let { child_user_ids } = request.joiData;
    const { User: Self } = request;

    child_user_ids = (child_user_ids || []).map((id) => ObjectId(id));

    if (child_user_ids.length) {
      if (Self.user_type_id != USER_TYPE_SUPER_ADMIN) {
        return resultResponse(
          NOT_FOUND,
          "Only Super Admin can pass child_user_ids",
        );
      }

      // validate child_user_ids
      const childUsers = await User.find({
        _id: { $in: child_user_ids },
        parent_id: ObjectId(Self._id)
      })
        .select({ _id: 1 }).lean().exec();

      if (child_user_ids.length != childUsers.length) {
        return resultResponse(NOT_FOUND, "Some of the Child User Ids are not valid");
      }

    }

    const query = getCLientPlSumQuery({
      child_user_ids: child_user_ids,
      self_user_id: Self._id,
    });

    const result = await User.aggregate(query);

    if (!result.length) {
      return resultResponse(NOT_FOUND, "Client Sum Data is empty");
    }

    const selfData = result.find((item) => item.user_id == Self._id) || {};
    const childData = result.filter((item) => item.user_id != Self._id);

    // Construct successful response
    return resultResponse(SUCCESS, {
      msg: "Client Sum Data fetched Successfully",
      data: {
        selfData,
        childData,
      },
...
function getCLientPlSumQuery(params) {

  const { child_user_ids, self_user_id } = params;
  const parentIds = [
    ObjectId(self_user_id),
    ...child_user_ids,
  ];
