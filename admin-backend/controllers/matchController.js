...
module.exports = class MatchController {
  ...
  static async createMatch(req, res) {
    ...
    try {
      ...
      return Responder.error(res, { msg: error.details[0].message })
    }
    try {
      let {
      ...
      try {
        if (market_id == undefined && market_name == undefined && runners == undefined) {
          try {
                ...
                if (onlineMarketLists.length) {
                  ...
                  for (const onlineMarketList of onlineMarketLists) {
                    ...
                    if (sport_id == CONSTANTS.HR && onlineMarketList.marketName.toLowerCase().includes(CONSTANTS.TO_BE_PLACED.toLocaleLowerCase())) {
                      try {
                        const apiRes = await axios.get(GET_ODDS_API_DELAY + onlineMarketList.marketId,
                          { timeOut: 2000 });
                    ...
                    let eventsWiseLimitesData = await EventsWiseLimites
                      .findOne({ series_id, event_type: 'market', market_name: onlineMarketList.marketName })
                      .select([
                        "-_id",
                        "market_min_stack",
                        "market_max_stack",
                    ...
                    if (eventsWiseLimitesData) {
                      Object.assign(marketData, eventsWiseLimitesData);
                    }
                    ...
                    marketService.setBBBRates(onlineMarketList);
            ...
            await match.findByIdAndRemove(matchModel._id);
            return ResError(res, { msg: "Something went wrong in Market(Match Odds) creation : " + error.message });
          }
        } else {
      ...
      return ResError(res, { msg: "Something went wrong in match creation : " + error.message });
    }
  }
      ...
      .then(async body => {
        ...
        if (!isListOnly && user_type_id == USER_TYPE_SUPER_ADMIN) {
          ...
          let getAllMatchesFromDB = await matchService.getAllMatches(Filter, {
            ...
            is_auto_fancy_result: 1, is_auto_market_result: 1,
          });
  ...
  static async totaluserCount(req, res) {
    return matchService
      .totaluserCount(req)
      .then(result => {
        if (result.statusCode != SUCCESS) {
