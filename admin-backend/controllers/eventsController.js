const { ObjectId } = require("bson")
  ...
  , eventsWiseLimitsService = require('../service/event/eventsWiseLimitsService')
...
module.exports = {
  ...
  updateLimites: function (req, res) {
    ...
    return Joi.object({
      ...
      values: Joi.object({
        ...
        is_series_limit_applicable: Joi.boolean().optional(),
            ...
            return Promise.all(Models)
              .then(async limitOfMarketFancy => {
                if (limitOfMarketFancy.toString()) {
                  ...
                  return ResError(res, { msg: "Events not found!" });
              }).catch(error => ResError(res, error));
          } else
            return ResError(res, { msg: "User settings not found!" });
        }).catch(error => ResError(res, error));
      }).catch(error => {
        if (error.hasOwnProperty("details"))
          return ResError(res, { msg: error.details.map(data => data.message).toString() })
  ...
  update: function (req, res) {
    return Joi.object({
      ...
      update: Joi.object()
        .or("is_visible", "match_tv_url", "has_tv_url", "match_scoreboard_url", "news", "is_auto_fancy_result", "is_auto_market_result").required(),
    }).validateAsync(req.body, { abortEarly: false })
  ...
  //Market wise limits
  updateMarketWiseLimites: function (req, res) {
    return eventsWiseLimitsService
      .updateMarketWiseLimites(req)
      .then((result) =>
  ...
  //Get limits
  getMarketWiseLimites: function (req, res) {
    return eventsWiseLimitsService
      .getMarketWiseLimites(req)
      .then((result) =>
        result.statusCode == SUCCESS
          ? ResSuccess(res, { validations: VALIDATION, data: result.data })
          : ResError(res, { msg: result.data })
      )
      .catch((error) => ResError(res, error));
  },

  //Add new market and fancy name
  addOrUpdateMarketFancyCatName: function (req, res) {
    return eventsWiseLimitsService
      .addOrUpdateMarketFancyCatName(req)
      .then((result) =>
        result.statusCode == SUCCESS
          ? ResSuccess(res, result.data)
          : ResError(res, { msg: result.data })
