...
module.exports = {
  ...
  createLogoAndBackground: async function (req, res) {
    try {
      ...
      } catch (error) { return ResError(res, error) }
    }
    catch (error) {
      return ResError(res, error);
    }
      ...
      return ThemeSetting.findOne({ domain: websiteSetting._id })
        .then(websiteData => {
          if (websiteData && websiteData != null && websiteData != '{}')
            ...
            return ResError(res, { msg: "Theme settings not available!" });
        }).catch((error) => ResError(res, error));
    }
  },
    ...
    return Joi.object({
      ...
      .then(async data => {
        ...
        return Content.findOne(Filter).select("-_id description self_host").lean()
          .then(data => data ? ResSuccess(res, { data }) : ResError(res, { msg: "No content found!" })).catch(error => ResError(res, error));
      }).catch(error => {
        if (error.hasOwnProperty("details"))
          return ResError(res, { msg: error.details.map(data => data.message).toString() })
        return ResError(res, error);
  ...
  contentGet: async function (req, res) {
    if (!req.query.content_type) {
      return ResError(res, { msg: "Content type is required." });
    }
    ...
    try {
      switch (contentType) {
        ...
        case 'Popup':
          ...
          let is_default_popup = false;
          if (getWebsiteSettings.statusCode == SUCCESS) {
            is_default_popup =
              getWebsiteSettings?.data?.is_default_popup || false;
          }
          if (!is_default_popup) {
  ...
  uploadPopupContent: async function (req, res) {
    ...
    const isMobile = data?.content_for == 'mobile';

    if (isMobile) {
      // Move mobile-specific fields
      data.content_mobile = data.content;
      delete data.content;

      if (data.content_meta) {
        data.content_meta_mobile = data.content_meta;
        delete data.content_meta;
      }
    }

    if (
      req.User.user_type_id == USER_TYPE_SUPER_ADMIN ||
      req.User.user_type_id == USER_TYPE_WHITE_LABLE
    ) {
      if (req.User.user_type_id == USER_TYPE_WHITE_LABLE) {
        const userDomain = req.User.domain_name;
      ...
      const content = await contentService.findContent(filter);
      return Content.findOneAndUpdate(filter, data, {
        upsert: true,
        new: true,
        runValidators: true
      })
        ...
        .then(() => {
          try {
            if (content.statusCode == SUCCESS) {
              const removeOldContentPath = isMobile
              ...
              if (cloudUploadService.isEnabled()) {
                const uploadData = {
                  ...content.data,
                  content: isMobile ? content.data.content_mobile : content.data.content,
                  content_meta: isMobile
                    ? content.data.content_meta_mobile
                    : content.data.content_meta
                };

                cloudUploadService
                  .removeImageFromCloud(uploadData)
                  .then()
                  .catch((error) => ResError(res, { error }));
              }
            }
          } catch (error) {
            console.error("Error in content removal", error);
          }

          return ResSuccess(res, {
            msg: `${data.content_type} content updated successfully...`
          });
        })
        .catch((error) => {
          return ResError(res, { error });
