const _ = require('lodash')
  ...
  , marketRollbackList = require('../service/rollBackService/marketRollbackList')
...
const { GET_ODDS_API_DELAY, } = require('../../config/constant/rateConfig');
...
module.exports = {
  async createMarket(req, res) {
    ...
    try {
          ...
          if (onlineMarketRes.length) {
            ...
            if (onlineMarketRes != undefined) {
              ...
              if (sport_id == CONSTANTS.HR && onlineMarketRes.marketName.toLowerCase().includes(CONSTANTS.TO_BE_PLACED.toLocaleLowerCase())) {
                try {
                  const apiRes = await axios.get(GET_ODDS_API_DELAY + onlineMarketRes.marketId,
                    { timeOut: 2000 });
              ...
              marketService.setBBBRates(onlineMarketRes);

            }
          }
        } catch (error) {
          return ResError(res, { msg: `Error to create market, runners ${error.message} ${getCurrentLine.default().file.split(/[\\/]/).pop()}:${getCurrentLine.default().line}` });
        }
      }
      ...
      let eventsWiseLimitesData = await EventsWiseLimites
        .findOne({ series_id: isMatchDataExists.series_id, event_type: 'market', market_name })
        .select([
          "-_id",
          "market_min_stack",
          "market_max_stack",
        ...
        .lean()
        .exec();
      if (eventsWiseLimitesData) {
        Object.assign(req.body, eventsWiseLimitesData);
      }
      ...
      return ResError(res, { msg: 'Error to create Market ' + error.message });
    }
  },
  ...
  marketRollbackList(req, res) {
    return marketRollbackList.marketRollbackList(req, res)
      .then(result => (result.statusCode == SUCCESS) ? ResSuccess(res, result.data) : ResError(res, result.data))
      .catch(error => ResError(res, error));
  },
  createManualRates(req, res) {
    return marketService.createManualRates(req, res)
      .then(result => (result.statusCode == SUCCESS) ? ResSuccess(res, result.data) : ResError(res, result.data))
      .catch(error => ResError(res, error));
  },
}