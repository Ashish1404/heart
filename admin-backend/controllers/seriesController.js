...
module.exports = class SeriesController {
      ...
      .then(async body => {
        ...
        if (!active_only && user_type_id == USER_TYPE_SUPER_ADMIN) {
          ...
          let getAllSeriesFromDB = await seriesService.getAllSeries({ sport_id },
            { _id: 0, sport_id: 1, series_id: 1, name: 1, is_manual: 1, is_active: 1, is_visible: 1, is_created: 1, create_at: 1, match_count: 1, is_series_limit_applicable: 1 },
            { name: 1 }
          );
  ...
  static async getSeriesV1(req, res) {
    try {
      ...
      return Responder.error(res, { msg: `Something went wrong! : ${error.message}` });
    }
  }
