                ...
                try {
                    ...
                    if (!existingMobile) {
                        ...
                        if (response.status == 200) {
                            ...
                            return ResError(res, { msg: response.response.data.message });
                        }
                    } else {
                        const response = await sendCode(data);
                        if (response.status == 200) {
                    ...
                    return ResError(res, { msg: err.message });
                }
            })
            .catch((error) => {
            ...
            .then(({ orderId }) => {
                return NotVerifiedMobileNo.findOne({ orderId: orderId })
                    ...
                    .then(async (user) => {
                        if (!user) {
                            return ResError(res, {
                                msg: "Order ID not found, and OTP can't be resent.",
                            });
                        }
                        if (user.is_verified != undefined && user.is_verified == 1) {
                            return ResError(res, {
                                msg: "Mobile Number already verified on the account!",
                            });
                        }
                        if (!user.mobile && !user.country_code) {
                            return ResError(res, { msg: "Mobile Number not found!" });
                        }
                        ...
                        await verifyCode(req)
                            .then(async (response) => {
                                if (
                                    ...
                                    return ResError(res, { msg: "Incorrect OTP!" });
                                }
                            })
                            .catch((err) => {
                                return ResError(res, { msg: err.message });
