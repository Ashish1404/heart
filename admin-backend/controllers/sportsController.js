...
module.exports = {
  ...
  createNewSport: async function (req, res) {
    ...
    return Sports.create(req.body)
      ...
      .catch(error => Responder.error(res, { msg: error.message }));
  },
  // To create new sport
      ...
      .then(async body => {
        ...
        if (user_type_id == USER_TYPE_SUPER_ADMIN) {
          ...
          if (!getAllSportsFromAPI.length || [NOT_FOUND, SERVER_ERROR].includes(getAllSportsFromDB.statusCode))
            return ResError(res, { dataIs: NAME, msg: `No Sports available yet!, ${getAllSportsFromDB.data}` });
        } else {
          ...
          return Sports.find({
            ...
            .then(async events => {
              if (events.length) {
                ...
                return ResError(res, { msg: "No sports available yet!" });
            });
        }
      }).catch(error => {
