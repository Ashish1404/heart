    ...
    return Joi.object({
      ...
      .then(async data => {
        ...
        if (getDepositStatus) {
          ...
          return ResError(res, { msg: "Your previous request is being processing..." });
        }

        await publisher.set(duplicateDepositPrevent, new Date(), 'EX', EXPIRE).then();

        if (!req.file)
          return ResError(res, { msg: "Please upload the payment screenshot!" });
        var file = req.file;
        ...
        if (checkexistRequest.data) {
          removeStaticContent(req.file.path);
          return ResError(res, { msg: "Your previous request is being processed!" });
        }
        ...
        if (amount > userDetails.balance && crdr === DEBIT_TWO)
          return ResError(res, { msg: "Insufficient Balance!", status: false });
    ...
    return Joi.object({
      ...
      .then(async data => {
        ...
        if (valueExistMethod.data.length == 0) {
          return ResError(res, { msg: "please add atleast one payment method!", status: false });
        }
        if (checkexistRequest.data) {
          return ResError(res, { msg: "Your previous request is under process!", status: false });
        }
        amount = parseFloat(amount);
        let description = `Transaction By ${req.User.name}(${req.User.user_name})`;
        if (amount > userDetails.balance && crdr === DEBIT_TWO)
