...
module.exports = class WebsiteSettingController {
  ...
  static async updateDomainNewToOld(req, res) {
    try {
      ...
      console.log(err, 'err')
      console.error(`Error updating domain from ${req.body.old_domain} to ${req.body.new_domain}: ${err}`);
      return Responder.error(res, err);
    }
  }
  ...
  static async getDomainWiseCounts(req, res) {
    try {
      ...
      console.error(`Error getting domain-wise counts: ${err}`);
      return Responder.error(res, err);
    }
  }
  ...
  static allowDefaultPopUpSetting(req, res) {
    return websiteService
      .updateDefaultPopUpSetting(req, res)
      .then((result) =>
        result.statusCode == SUCCESS
          ? ResSuccess(res, result.data)
          : ResError(res, { msg: result.data })
