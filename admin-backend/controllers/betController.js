...
module.exports = class BetController {
  ...
  static bets(req, res) {
    return Joi.object({
      ...
      limit: Joi.number().min(25).default(200).optional(),
      ...
      .then(data => {
        ...
        return BetResults.findOne(
          ...
          if (betResultAlreadyDeclared != null)
            return ResError(res, { msg: "Result already declared!" });
          ...
          return new Promise(async function (resolve, reject) {
            ...
            return ResError(res, { msg: error });
          });
        }).catch(error => ResError(res, { msg: `Error while getting result: ${error.message}` }));
      }).catch(error => {
        if (error.hasOwnProperty("details"))
          return ResError(res, { msg: error.details.map(data => data.message).toString() })
        return ResError(res, error);
        ...
        return betService.processOddsRollback({ market_id })
          .then(processOddsRollback => {
            if (processOddsRollback.statusCode != SUCCESS) {
              return ResError(res, { msg: processOddsRollback.data });
            }
          ...
          }).catch(error => ResError(res, { msg: `Error while rollback the market result: ${error.message}` }));
      }).catch(error => {
        if (error.hasOwnProperty("details"))
          return ResError(res, { msg: error.details.map(data => data.message).toString() })
        return ResError(res, error);
  ...
  static oddsAbandoned(req, res) {
    ...
    return Market.findOne(
      ...
      if (market != null) {
        ...
        }).catch(error => ResError(res, { msg: `Error while abandon the market result: ${error.message}` }));
      } else {
    ...
    return Fancy.findOne(
      ...
      if (fancy != null) {
        ...
        return betService.sessionAbandoned(Object.assign(fancy, { fancy_id, rollback })
          ...
          ResError(res, { msg: `Error while abandon the fancy result: ${error.message}` })
        );
      } else {
      ...
      .then(async data => {
        ...
        return betService.getExposuresV2({ user_id, userTypeId, isOnlyExposure }).then(betData => {
          if (betData.statusCode == NOT_FOUND)
            return ResError(res, { status: true, msg: betData.data, data: {} });
          else if (betData.statusCode == SERVER_ERROR)
            return ResError(res, { msg: betData.data });
