...
module.exports = class UserController {
				...
				if (isUserDetailsExist.statusCode == SUCCESS) {
					...
					if (mobile && country_code)
						if (isUserDetailsExist.data.mobile == mobile && isUserDetailsExist.data.country_code == country_code && req.User?.belongs_to_b2c) {
							return ResError(res, { msg: "Mobile no. already exists!" });
						}
					if (email)
						if (isUserDetailsExist.data.email == email)
							return ResError(res, { msg: "Email address already exists!" });
				}
				// check domain exists.
						...
						if (user_type_id != USER_TYPE_USER) {
							for (const sport of sports_share) {
								...
								if (!isValidParentIdOfLastSportPercentage.hasOwnProperty("parent_id"))
									return ResError(res, { msg: "sport.percentage.parent_id : Invalid parent_id property!" });
								...
								if (isValidParentIdOfLastSportPercentage.parent_id != (parentData._id).toString())
									return ResError(res, { msg: "sport.percentage.parent_id : Agent creation in wrong parent_id!" });
								...
								if (fieldHave.hasOwnProperty("user_type_id") || fieldHave.hasOwnProperty("user_name") || fieldHave.hasOwnProperty("user_id"))
									return ResError(res, { msg: "sport.percentage : It doesn't meet the requirements!" });
								if (isWhiteLabel) {
									if (sport_percetage.length != 2)
										return ResError(res, { msg: "Agent creation are not allowed in Super Admin. It doesn't meet the requirements!" });
									if (sport_percetage.length == 2)
										if (fieldHave.hasOwnProperty("user_type_id") || fieldHave.hasOwnProperty("user_name") || fieldHave.hasOwnProperty("user_id"))
											return ResError(res, { msg: "sport.percentage : Super -> Agent It doesn't meet the requirements!" });
								}
								sport_percetage.pop();
								let usersInvalid = [];
								for (const [index, percentage] of sport_percetage.entries()) {
									...
									if (index == 0)
										if (percentage.user_type_id != USER_TYPE_SUPER_ADMIN)
											return ResError(res, { msg: "sport.percentage.user_type_id : It doesn't meet the requirements!" });
									if (user_type_id == USER_TYPE_USER)
										return ResError(res, { msg: "sport.percentage.user_type_id : This user type not valid!" });
								...
								if (usersInvalid.length)
									return ResError(res, { msg: `${usersInvalid.toString()} user(s) or its details not match!` });
								if (parentData.user_type_id != USER_TYPE_SUPER_ADMIN) {
									...
									for (const [index, validParents] of parent_level_ids.entries()) {
										...
										if (
											...
											return ResError(res, { msg: "sport share parent not match!" });
									}
								}
								if (Math.max(parentData.self_lock_user, parentData.parent_lock_user) == 1)
									return ResError(res, { msg: "Your account is locked!" });
								if (Math.max(parentData.self_close_account, parentData.parent_close_account) == 1)
									return ResError(res, { msg: "Your account is closed!" });
						...
						if (isUkraineConceptRequest) {
							...
							if (user_type_id == USER_TYPE_USER) {
								...
								if (parentData.partnership > partnership) {
									return ResError(res, { msg: "Please Enter Valid Partnership." });
								}

							}

							if (opening_balance && parentData.balance < opening_balance) {
								return ResError(res, { msg: `Please refill the parent balance with ${opening_balance - parentData.balance}` });
							}
	...
	static async adminLogins(req, res) {
		...
		try {
			...
			return Responder.success(res, { msg: error.details[0].message })
		}
			...
			.then((user) => {
				if (user) {
						...
						if (!domainName.includes("localhost")) {
							if (Array.isArray(checkIsValidDomain(domainName))) {
								if (user.domain_name != domainName)
									return Responder.error(res, { msg: "You are not allowed to login!" });
							} else
								return Responder.error(res, { msg: "ip login not allowed!" });
						}
					}
					if (user.user_type_id == 1)
						return Responder.success(res, { msg: "Please login on user application." })
					...
					if (user.self_lock_user == 1 || user.parent_lock_user == 1)
						return Responder.success(res, { msg: "Your account is locked." })
					if (user.self_close_account == 1 || user.parent_close_account == 1)
						return Responder.success(res, { msg: "Your account is closed , Contact your Upline !!." })
						...
						.then((resp) => {
							...
							oauth.token(request, response)
								.then(function (token) {
									...
									return Responder.success(res, { msg: err })
								});
						})
				} else
...
*/
	...
	*/
							...
							await OAuthToken.deleteMany({ 'user.user_id': userId.toString() })
								.then(() => {
									User.updateOne({ _id: userId }, {
										...
										.then(() => ResSuccess(res, { msg: "Successfully disabled..." })).catch(error => ResError(res, error));
								});
						} else {
							ResError(res, { msg: "Invalid OTP!" })
						}
					})
			}).catch(error => {
				if (error.hasOwnProperty("details"))
	...
	static adminLogout(req, res) {
		...
		if (!accessToken)
			return ResError(res, { msg: "Token not passed!" });
		...
		User.findOne({ user_name: req.body.user_name.toLowerCase() })
			...
			.then((usrData) => {
				if (usrData && usrData != null && usrData != '{}')
					return Responder.success(res, { msg: "Username is already exists. " })
				else
					return Responder.success(res, { msg: "Username is available. ", })
			}).catch((err) => Responder.error(res, err))
	}
	...
	static async updateChildPassword(req, res) {
		...
		User.updateOne({ _id: req.body.childUserId, "parent_level_ids.user_id": (req.User.user_id || req.User._id) },
			...
			.then((val) => {
				...
				// Logout user
				OAuthToken.deleteMany({ 'user.user_id': req.body.childUserId.toString() }).then();
				// Update activity log status.
				updateLogStatus(req, { status: LOG_SUCCESS, msg: msg })
	...
	static async updatePassword(req, res) {
		return Joi.object({
			...
			.then(({ user_id, newPassword }) => {
				...
				let selfUser = (user_id).toString() == (req.User.user_id || req.User._id).toString()
				...
				return User.findOneAndUpdate(
					{ _id: user_id },
					{
						$set: {
							...
							// raw_password, 
							is_change_password: selfUser ? 1 : 0,
						}
					}
				).select("user_name belongs_to_credit_reference mobile is_change_password").lean()
					.then(async user => {

						let comment = "Password Changed By Self.";
	...
	static async updatePartnershipList(req, res) {
		...
		if (!req.body.sports_share)
			return Responder.success(res, { msg: "Sports share required" })
	...
	static async searchUser(req, res) {
		...
		try {
			...
			return Responder.success(res, { msg: 'Error in api!' })
		}
	}
	...
	static async getDailyBonusAmount(req, res) {
		...
		if (checkexistRequest && checkexistRequest.data) {
			return ResError(res, { data: { created_at: checkexistRequest.data.nextBonusClaimDate }, msg: `You have already claimed the daily bonus within the last 24 hours.You can try ${checkexistRequest.data.nextBonusClaimDate}.`, status: false });
		}
