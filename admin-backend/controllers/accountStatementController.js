...
module.exports = class AccountStatementController {
  ...
  static async makeSettlement(req, res) {
    try {
      if (req.user.user_type_id == USER_TYPE_SUPER_ADMIN
        ...
        return ResError(res, { msg: "You are not allowed!" });
      }
  ...
  static async makeSettlementDiamond(req, res) {
    try {
      ...
      if (req.User.user_type_id != USER_TYPE_SUPER_ADMIN &&
        req.User.belongs_to != LABEL_DIAMOND) {
        return ResError(res, { msg: "This Operation is not allowed to you." })
      }
  ...
  static async diamondChipSummary(req, res) {
    try {
      const response = await accountStatementService.diamondChipSummary(req);
  ...
  static async diamondSettlementHistory(req, res) {
    try {
      const response = await accountStatementService.diamondSettlementHistory(req);
