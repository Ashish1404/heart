...
let Fields = {
  ...
  self_blocked: 1,
  parent_blocked: 1,
  is_series_limit_applicable: 1,
  is_auto_fancy_result: 1,
  is_auto_market_result: 1,
}, EventMatch = [
      ...
      try {
        ...
        if (documentKey != undefined) {
          if (updateDescription != undefined) {
            ...
            if (updatedFields.hasOwnProperty("is_auto_fancy_result") && fieldsLength == 2) {
              isUpdated = true;
              isFancyUpdateOnly = true;
              matchFilter["match_id"] = 1;
            }
            if (updatedFields.hasOwnProperty("is_auto_market_result") && fieldsLength == 2) {
              isUpdated = true;
              matchFilter["match_id"] = 1;
            }
            if (isUpdated) {
              Match.findById(documentKey, matchFilter).lean().then(match => {
                ...
                if (
                  ...
                  updatedFields.hasOwnProperty("is_auto_fancy_result")
                  // || updatedFields.hasOwnProperty("parent_blocked") ||
                  // updatedFields.hasOwnProperty("self_blocked")) 
                  && fieldsLength == 2) {
                  update = { $set: {}, $addToSet: {}, $pull: {} };
                  if (!updatedFields.hasOwnProperty("is_auto_fancy_result")) {
            ...
            if (fullDocument != undefined) {
              ...
              Match.findOne({ match_id }, Fields).lean().then(match => {
                ...
                if (match?.is_series_limit_applicable) {
                  ...
                  // Remove the keys
                  keysToRemove.forEach(key => delete match[key]);
                }
        ...
        if (operationType != undefined) {
          if (operationType == "insert") {
            if (fullDocument != undefined) {
              ...
              Match.findOne({ match_id }, Object.assign(Fields, { session_category_limites: 1, is_series_limit_applicable: 1 }))
                .lean()
                .then(match => {
                  if (!match.is_series_limit_applicable) {
