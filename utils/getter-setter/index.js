...
module.exports = {
  ...
  getBallByBallBetUID: (user_id, roundId, betDate) => {
    const key = `${KEY.BALLBYBALL_BET_PLACE}_${user_id}_${roundId}_${betDate}`;
    return key;
  },
  ...
  getDmScoreUID: (eventId) => {
    const key = `${KEY.DM_SCORE}${eventId}${CONSTANTS.UNIQUE_IDENTIFIER_KEY}`;
    return key;
  },
}