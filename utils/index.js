...
const {
  FETCH_DATA_FROM_FOR_FANCY,
  FETCH_DATA_FROM_FOR_MARKET,
} = require('../config/constant/rateConfig');
...
module.exports = {
  ...
  checkDomain: async function (req) {
    if (ALLOW_DOMAINS_TO_BY_PASS.includes("*")) return "";

    let domainName = module.exports.getDomainName(req.get('host'));
    if (ALLOW_DOMAINS_TO_BY_PASS.split(",").includes(domainName)) {
      return "";
    }
