const cron = require('node-cron')
  ...
  , archerService = require('../admin-backend/service/ballbyball/ballByBallService')
...
let marketResultRunning = false, fancyResultRunning = false, qtechResultRunning = false, universeCasinoResultRunning = false
  ...
  , dmScoreBoardRunning = false
  , archerAutoClearLiabilityRunning = false
  , archerResultSettlementRunning = false
  , marketResultParams = { search: { is_rollback: 0, is_processing: 0 }, pendingMarkets: 1, page: 1, limit: 50, is_auto_market_result: true }
  , fancyResultParams = { search: { is_result_declared: 0, is_active: 0, is_rollback: 0, is_processing: 0, is_auto_fancy_result: true }, limit: 25, page: 1 };
...
module.exports = {
  ...
  oddsService: () => {
    ...
    async function autoMinMaxLimitUpdateDmProvider() {
      try {
        await marketService.autoMinMaxLimitUpdateDmProvider();
      } catch (error) {
        console.error(error);
      }
    }
  ...
  archerResultSettlement: () => {
    /*This Cron Dumps the active Markets data to Redis*/

    async function archerResultSettlementInner() {

      if (archerResultSettlementRunning)
        return;

      archerResultSettlementRunning = true;

      try {
        const startTime = moment();

        await archerService.resultSettlement();
        const msg = `Archer Result Settlement: ${getTimeTaken({ startTime })}`;

        // logger.info(msg);
        // console.log(msg)
      } catch (error) {

        logger.info(`ERROR In archerResultSettlementInner
          FILE: ${getCurrentLine.default().file}:${getCurrentLine.default().line}
          FUNCTION: ${getCurrentLine.default().method}
          ERROR: ${JSON.stringify(error)}
          ERROR_STACK: ${JSON.stringify(error.stack)}
    ...
    // Run every 30 Secs
    console.info("Archer Result Settlement Started...");
    // archerResultSettlementInner()
    setInterval(() => archerResultSettlementInner(), 1000 * 30);
  },
  archerAutoClearLiability: () => {
    /*This Cron Dumps the active Markets data to Redis*/

    async function archerAutoClearLiabilityInner() {

      if (archerAutoClearLiabilityRunning)
        return;

      archerAutoClearLiabilityRunning = true;

      try {
        const startTime = moment();

        await archerService.autoClearLiability();
        const msg = `Archer Auto Clear Liability: ${getTimeTaken({ startTime })}`;

        // logger.info(msg);
        // console.log(msg)
      } catch (error) {

        logger.info(`ERROR In archerAutoClearLiabilityInner
          FILE: ${getCurrentLine.default().file}:${getCurrentLine.default().line}
          FUNCTION: ${getCurrentLine.default().method}
          ERROR: ${JSON.stringify(error)}
          ERROR_STACK: ${JSON.stringify(error.stack)}
    ...
    // Run every 5 Mins
    console.info("Archer Auto Clear Liability started...");
    setInterval(() => archerAutoClearLiabilityInner(), 1000 * 60 * 5);

  },
  dmScoreBoard: () => {
    /*This Cron Dumps Score Data To Redis*/

    async function dmScoreBoardInner() {

      if (dmScoreBoardRunning)
        return;

      dmScoreBoardRunning = true;

      try {
        const startTime = moment();

        const res = await matchService.fetchAndSaveScoreData();
        const msg = `DM ScoreBoard: ${getTimeTaken({ startTime })}`;

        // logger.info(msg);
        // console.log(msg)
      } catch (error) {

        logger.info(`ERROR In dmScoreBoardInner
          FILE: ${getCurrentLine.default().file}:${getCurrentLine.default().line}
          FUNCTION: ${getCurrentLine.default().method}
          ERROR: ${JSON.stringify(error)}
          ERROR_STACK: ${JSON.stringify(error.stack)}
    ...
    if (ODDS_API_PROVIDER == 'DM_PROVIDER') {
      // Run every 3 Sec
      console.info("DM ScoreBoard started...");
      setInterval(() => dmScoreBoardInner(), 1000 * 3);
    }
  },
}