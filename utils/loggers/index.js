...
const ArcherResultFilter = format((info, opts) => {

  if (process.env.SHOW_DEV_LOGS === 'true') {
    console.info(LOG_DIR_PATH);
  }

  return info.level === "ArcherResult" ? info : false;
});
...
const logger = createLogger({
  ...
  transports: [
    ...
    new DailyRotateFile({
      filename: `archer-result-%DATE%.log`,
      dirname: `${LOG_BASE_DIR_PATH}archerResult/`,
      datePattern: 'YYYY-MM-DD',
      frequency: "24h",
      level: 'ArcherResult',
      maxSize: "100m",
      format: combine(ArcherResultFilter()),
      zippedArchive: true // Compress old logs
    }),
