...
async function transactions(request) {
  ...
  const isResult = body.completed == 'true';
  let ccId;

  if (isResult) {

    // Check Concurrency 
    const betPlaceKey = getAuraUID('QTECH_RESULT:', body.roundId, body.playerId);
    ...
    if (ccResponse.statusCode == SERVER_ERROR) {
      return resultResponse(NOT_FOUND, {
        status: QT.STATUS_400,
        data: { "code": QT.REQUEST_DECLINED, "message": "Result already in progress..." }
      });
    }
