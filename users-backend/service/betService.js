...
const {ODDS_API_PROVIDER} = require('../../config/constant/rateConfig')
const { getBetDelay } = require("../../admin-backend/service/betService/getBetDelay");
const { addUpdateBetCount } = require('../../admin-backend/service/betCount/betCountService');
  ...
  try {
    ...
    return userService.getUserDetails(
      ...
      if (user.statusCode == SUCCESS) {
        ...
        let inplay = getMarketInplay.data;
        if (ODDS_API_PROVIDER == 'DM_PROVIDER' && market.market_type == BOOKMAKER_TYPE) {
...
let validateBetAndRedisOddsWhileBetPlacing = (data) => {
  return exchangeService.getOddsRate(data).then(async betFairOdss => {
    ...
    try {
      ...
      if (data?.market?.is_series_limit_applicable && !data.market.inplay) {
        if (userMaxProfit > data?.market?.market_before_inplay_profit && data?.market?.market_before_inplay_profit != 0)
          return resultResponse(VALIDATION_ERROR, `Market max profit(${data?.market?.market_before_inplay_profit}) limit is over`);
      }
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      try {
        ...
        if (!data?.is_demo && !data?.is_auto_demo) {
          Market.updateOne(
          ...
          data.event_name = data.market_name;
...
let validateFancyBetLiabilityBeforeBetPlace = async (data) => {
  ...
  data.profit = Math.round(p_l);
  data.liability = -(Math.round(liability));
  data.stack = stack;
  data.size = size;
    ...
    await session.withTransaction(async () => {
      ...
      try {
        ...
        if (!data?.is_demo && !data?.is_auto_demo) {
          ...
          data.event_name = data.fancy_name;
          addUpdateBetCount(data);
        }
      } catch (error) {
        throw new Error(`${BET_NOT_PLACED} ` + (process.env.DEBUG == "true" ? `UU_E ${error.message}` : ''));
