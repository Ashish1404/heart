...
module.exports = {
			...
			.then(params => {
				...
				return User.findOne(userQuery).select("user_type_id belongs_to_b2c").sort({ createdAt: 1 }).limit(1).lean().then(user => {
					if (user) {
						...
						return ResError(res, { msg: "No upline master found yet!" });
					...
					.then(async user => {
						if (user) {
							if (user.user_type_id != USER_TYPE_USER)
								return ResError(res, { msg: "Please login into agent panel!" });
							...
							if (!passwordCheck) { // If password is not vailid.
								...
								return ResError(res, { msg: loginUserLog.message });
							}
								...
								return ResError(res, { msg: "Your account is closed, Contact your Upline!" });
							else {
								...
								if (user.is_telegram_enable) {
									if (!user.telegram_chat_id)
										return ResError(res, { msg: "On this account telegram not associated!" });
							...
							return ResError(res, { msg: "Invalid credentials! Please try again." });
					}).catch(error => ResError(res, error));
			}).catch(error => {
				if (error.hasOwnProperty("details"))
					return ResError(res, { msg: error.details.map(data => data.message).toString() });
					...
					.then(async user => {
						if (user) {
							...
							if (user.expire_time < Date.now())
								return ResError(res, { msg: "OTP has been expired!" });
							user.is_online = 1;
							user.sessionid = undefined;
	...
	updateForChangePassword: async function (req, res) {
		...
		if (!req.params.id)
			return Responder.success(res, { msg: "User id is required." })
			...
			.then((fnd) => {
				if (fnd) {
					...
					if (!req.body.old_password) {
						return Responder.success(res, { msg: "Please enter old password" })
					}

					if (!req.body.new_password) {
						return Responder.success(res, { msg: "Please enter new password" })
					}

					if (!req.body.confirm_password) {
						return Responder.success(res, { msg: "Please enter confirm password" })
					}

					if (req.body.confirm_password != req.body.new_password) {
						return Responder.success(res, { msg: "Password and confirm password do not match" })
					}
	...
	myProfile: async function (req, res) {
		return userService.getUserDetails(
			...
			return ResError(res, { msg: user.data });
		}).catch(error => ResError(res, error));
	},
	// Ukraine Concept
